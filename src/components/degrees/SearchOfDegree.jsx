...
function SearchOfDegree() {
    ...
    return (
        <section id="search-degree" className="text-gray-900 body-font bg-[#800000]/5">
            <div className="mx-auto px-5 py-12">
                <div className="flex flex-col text-center w-full mb-12">
                    ...
                    <p className="lg:w-2/3 mx-auto leading-relaxed text-base text-gray-900">Find the perfect degree program that suits your interests and career goals. Use the filters below to narrow down your search.</p>
                </div>
                                    ...
                                    <div className="p-4 grid grid-cols-1 gap-4">
                                        {['bachelors', 'masters', 'diploma', 'certification'].map(filter => (
                                            <label key={filter} className="flex items-center">
                                                ...
                                                />
                                                <span className="ml-2 text-gray-900">{filter.charAt(0).toUpperCase() + filter.slice(1)}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        ...
                        {Array.from({ length: endPage - startPage + 1 }, (_, i) => startPage + i).map(number => (
                            <button
                                ...
                                className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${currentPage === number ? 'bg-[#800000] text-white ' : 'bg-white text-gray-900'} border-gray-300 `}
                            >
                                {number}
                            </button>
                        ))}
